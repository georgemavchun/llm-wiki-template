...
1. **Quarantine tier**: a "never enters the repo" list in the schema (credentials, government IDs, card data, health, compensation, legal privilege, explicit off-record). `privacy_preflight.py` detects the deterministic subset; exit `2` blocks; output never echoes matched values. An owner-approved allowlist (`wiki/privacy-allowlist.json`, bound to the file's SHA-256 and a reason code) records accepted false positives; `--no-allowlist` audits without it. (Added after the first end-to-end run: without it a false positive could only be committed with `--no-verify`.)
...
4. **Raw immutability**: `.claude/settings.json` denies `Edit(./wiki/raw/**)`; `guard_write.py` (PreToolUse) blocks Write/Edit on existing raw files and blocks any Write/Edit whose content trips the scanner; git `pre-commit` refuses modifications, deletions or renames under `wiki/raw/` outside the `raw/inbox/` drop zone unless `WIKI_ALLOW_RAW_CHANGE=1` (documented redaction override; the inbox exemption was added after the end-to-end run showed git records the documented inbox-to-raw `mv` as a rename), refuses anything under `.staging/`, and scans staged wiki files; CI repeats the raw add-only and scan checks.
